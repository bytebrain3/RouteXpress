...
class RouteXpress {
    constructor(config) {
        ...
        if (config.webhooks?.steadfast?.enabled && config.webhooks.steadfast.apiSecret) {
            this.steadfastWebhook = new SteadfastWebhookHandler(config.webhooks.steadfast.apiSecret);
        }
        if (config.webhooks?.pathao?.enabled && config.webhooks.pathao.integrationSecret) {
            this.pathaoWebhook = new PathaoWebhookHandler(config.webhooks.pathao.integrationSecret);
        }
        if (config.webhooks?.redx?.enabled && config.webhooks.redx.apiAccessToken) {
            this.redxWebhook = new RedXWebhookHandler(config.webhooks.redx.apiAccessToken);
        }
